...
class Inject():
	...
	def move_gadget_config_2_so_dir(self, lib):
		shutil_copy(os.path.join(CURRENT_SCRIPT_ABSOLUTE_PATH,GADGET_CONFIG_FILE_NAME),
			dst=os.path.join(self.p, lib))
...
if __name__ == '__main__':
	...
	parser.add_argument('-g',required=True,dest='gadir', help='存放所有FRIDA-GADGET文件的目录名 (必选)')
	parser.add_argument('-ng',dest='newgname',default='libhaiw2gt',help='所有FRIDA-GADGET文件的新名字 (默认: %(default)s)')
	parser.add_argument('-s',required=True,dest='sname', help='选择需要注入的SO文件名[默认注入所有架构] (必选)')
	parser.add_argument('-a',required=True,dest='audir', help='APK 解压后的文件夹名称 (必选)')
	
	args = parser.parse_args()
	if(args.newgname=='libhaiw2gt'):
		print('[*] Rename all gadget file name as the default "libhaiw2gt" - (Pretend)')

	f = lambda x: x.find('.so')!=-1?x[-3]:x
	if(args.audir.find('.apk')!=-1):
		audir = args.audir[:-3]
	else:
		audir = args.audir

	# import param
	GADGET_FILE_NEW_NAME, SO_FILE_NAME = map(f, [args.newgname, args.sname])
	GADGET_CONFIG_FILE_NAME = GADGET_FILE_NEW_NAME + '.config.so'
	SO_FILE_NAME = SO_FILE_NAME + '.so'
	GADGET_DIRECTORY_NAME = args.gadir
	APK_UNPACK_DIRECTORY_NAME = audir
